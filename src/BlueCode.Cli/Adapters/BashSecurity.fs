...
let private shellOperators: Set<char> = Set.ofList [ ';'; '|'; '&'; '<'; '>' ]
...
let private destructivePatterns: (Regex * string) list =
    [
      ...
      (Regex(@"\bgit\s+reset\s+--hard\b", RegexOptions.Compiled), "Note: may discard uncommitted changes")
      ...
      (Regex(@"\bgit\s+commit\b[^;&|\n]*--amend\b", RegexOptions.Compiled), "Note: may rewrite the last commit")
      // File deletion
      (Regex(
      ...
      (Regex(@"(^|[;&|\n]\s*)rm\s+-[a-zA-Z]*[rR]", RegexOptions.Compiled), "Note: may recursively remove files")
      (Regex(@"(^|[;&|\n]\s*)rm\s+-[a-zA-Z]*f", RegexOptions.Compiled), "Note: may force-remove files")
      // Database
      ...
      // Infrastructure
      (Regex(@"\bkubectl\s+delete\b", RegexOptions.Compiled), "Note: may delete Kubernetes resources")
      (Regex(@"\bterraform\s+destroy\b", RegexOptions.Compiled), "Note: may destroy Terraform infrastructure") ]
...
let private validateEmpty (ctx: ValidationCtx) : SecurityDecision =
    if ctx.OriginalCommand.Trim() = "" then
        Allow
    else
        Passthrough
...
let private validateGitCommit (ctx: ValidationCtx) : SecurityDecision =
    ...
    if
        ctx.BaseCommand <> "git"
        || not (Regex.IsMatch(ctx.OriginalCommand, @"^git\s+commit\s+"))
    then
        Passthrough
                ...
                let hasUnquotedRedirect =
                    if remainder = "" then
                        ...
                        for c in remainder do
                            if c = '\'' && not inDq then
                                inSq <- not inSq
                            elif c = '"' && not inSq then
                                inDq <- not inDq
                            elif not inSq && not inDq then
                                sb.Append(c) |> ignore
...
let private validateJqCommand (ctx: ValidationCtx) : SecurityDecision =
    if ctx.BaseCommand <> "jq" then
        ...
        let afterJq =
            if ctx.OriginalCommand.Length > 3 then
                ctx.OriginalCommand.[3..].TrimStart()
            else
                ""
...
let private validateDangerousVariables (ctx: ValidationCtx) : SecurityDecision =
    ...
    if
        Regex.IsMatch(fu, @"[<>|]\s*\$[A-Za-z_]")
        || Regex.IsMatch(fu, @"\$[A-Za-z_][A-Za-z0-9_]*\s*[|<>]")
    then
    ...
    if not (original.Contains('\n')) || not (original.Contains('#')) then
        ...
        while i < original.Length && decision = Passthrough do
            ...
            if escaped then
                ...
                let lineEnd = if nextNewline = -1 then original.Length else nextNewline
...
let private validateZshDangerousCommands (ctx: ValidationCtx) : SecurityDecision =
    ...
    let tokens = trimmed.Split([| ' '; '\t' |], StringSplitOptions.RemoveEmptyEntries)
    ...
    | _ ->
        ...
        let baseCmd =
            if trimmed = "" then
                ...
                trimmed.Split([| ' '; '\t' |], 2, StringSplitOptions.None).[0]
