...
let toLlmOutputTests =
    testList
        "ToLlmOutput"
        [

          testCase "final action with string answer -> FinalAnswer"
          <| fun () ->
              let step =
                  { thought = "done"
                    action = "final"
          ...
          <| fun () ->
              ...
              | Error(SchemaViolation detail) ->
                  Expect.isTrue
                      (detail.Contains("answer"))
                      "SchemaViolation detail should mention the missing 'answer' field"
              | other -> failtestf "Expected Error (SchemaViolation ...) but got: %A" other

          testCase "final action with non-string 'answer' (number) -> SchemaViolation"
          <| fun () ->
              let step =
                  { thought = "done"
                    action = "final"
          ...
          testCase "final action with non-string 'answer' (object) -> SchemaViolation"
          <| fun () ->
              let step =
                  { thought = "done"
                    action = "final"
          ...
          testCase "tool action -> ToolCall with _raw passthrough"
          <| fun () ->
              let step =
                  { thought = "reading"
                    action = "read_file"
              ...
              | Ok(ToolCall(ToolName name, ToolInput map)) ->
                  ...
                  Expect.isTrue
                      (Map.containsKey "_raw" map)
                      "Phase 2 tool input passes raw JSON string under _raw key (Phase 3 refines)"
              | other -> failtestf "Expected Ok (ToolCall ...) but got: %A" other ]
