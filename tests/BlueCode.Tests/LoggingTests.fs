...
let tests =
    testList
        ...
        [
          ...
          testCase "Default Information suppresses Log.Debug events"
          <| fun _ ->
              ...
              let debugCount =
                  captured |> Seq.filter (fun e -> e.Level = LogEventLevel.Debug) |> Seq.length
              ...
              Expect.equal debugCount 0 "Debug event should be suppressed at Information minimum level"
              Expect.equal infoCount 1 "Information event should pass through at Information minimum level"

          testCase "Flipping switch to Debug reveals Log.Debug events"
          <| fun _ ->
