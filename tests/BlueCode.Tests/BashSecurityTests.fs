...
let dangerousRedirectionTests = testList "dangerous redirections" [
...
let newlineInjectionTests = testList "newline separator" [
...
let unicodeWhitespaceTests =
    ...
    testList "unicode whitespace (BLOCK)" [
...
let backslashWhitespaceTests = testList "backslash-escaped whitespace (BLOCK)" [
...
let jqTests = testList "jq dangerous patterns (BLOCK)" [
...
let zshEqualsTests = testList "zsh equals expansion (BLOCK)" [
...
let shellMetacharTests = testList "shell metacharacters in quoted args (BLOCK)" [
