...
let tests =
    testList "Repl.shouldWarnContextWindow" [
        ...
        testCase "below 80% threshold -> no warning" <| fun _ ->
            ...
            Expect.isFalse
                (shouldWarnContextWindow 10000 8192 false)
                "10K chars of 32K budget should not warn"

        testCase "exactly at 80% boundary -> warn" <| fun _ ->
            ...
            Expect.isTrue
                (shouldWarnContextWindow 26215 8192 false)
                "26215 chars (at/above 80% boundary) should warn"

        testCase "above 80% threshold -> warn" <| fun _ ->
            Expect.isTrue
        ...
        testCase "small model (2048) fires warning earlier" <| fun _ ->
            ...
            Expect.isTrue
                (shouldWarnContextWindow 7000 2048 false)
                "7000 chars above 80% of 2048 token model"
    ]
