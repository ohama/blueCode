...
let private smokeTest () =
    testCaseAsync "live Qwen: 'list files' completes ≤5 steps with final answer"
    <| async {
        let tempRoot =
            Path.Combine(Path.GetTempPath(), sprintf "bluecode-smoke-%s" (Guid.NewGuid().ToString("N")))
        ...
        try
            ...
            | Error e -> failtestf "expected Ok result, got: %A (log: %s)" e c.LogPath
        finally
            (c.JsonlSink :> IDisposable).Dispose()
    }

let private disabledStub () =
    testCase "live Qwen smoke: disabled (set BLUECODE_AGENT_SMOKE=1 to enable)"
    <| fun _ -> ()

[<Tests>]
let tests =
    testList "AgentLoop.Smoke" [ if smokeEnabled () then smokeTest () else disabledStub () ]
