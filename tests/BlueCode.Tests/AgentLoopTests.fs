...
let agentLoopTests =
    testList
        ...
        [
          ...
          testCaseAsync "max iter: 5 distinct ToolCalls without FinalAnswer -> MaxLoopsExceeded"
          <| async {
              let calls =
          ...
          <| async {
              ...
              match result with
              | Error(LoopGuardTripped action) -> Expect.equal action "read_file" "tripped action should be read_file"
              | other -> failtestf "expected Error (LoopGuardTripped _), got %A" other
          }

          testCaseAsync "JSON retry pass: malformed first attempt, valid second -> Ok"
          <| async {
          ...
          testCaseAsync "JSON retry exhausted: both malformed -> InvalidJsonOutput with ORIGINAL raw"
          <| async {
              ...
              match result with
              | Error(InvalidJsonOutput raw) -> Expect.equal raw "garbage-1" "original (first) raw should be preserved"
